...
class GnuboardMcp
{
    ...
    public function basicSkinSnippet(string $type): array
    {
        $snippets = [
            ...
            'grade_color' => [
                'code' => '<span class="grade-<?php echo \$member[\\"mb_level\\"]; ?>"></span>',
                'hint' => '회원 등급별 스타일 적용',
...
<section id="anc_cf_{$pluginName}">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
<?php
$key = 'cf_'.$pluginName.'_option1';
?>
<input type="text" name="option1"
       value="<?php echo $config[$key] ?? '' ?>"
       id="option1" class="frm_input" size="40">
            </td>
        </tr>
        </tbody>
        </table>
...
// 서버 실행
use PhpMcp\Server\Transports\HttpServerTransport; // <--- 이 줄로 변경하거나, 기존 use 문에 추가합니다.
...
try {
    ...
    $transport = new HttpServerTransport(); // <--- 이 줄로 변경합니다.
